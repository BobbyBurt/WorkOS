...
export default class medalScene extends Phaser.Scene {
	...
	editorCreate(): void {
		...
		// difficultyText
		const difficultyText = this.add.bitmapText(
			68,
			6,
			"featured-item-font",
			"difficult"
		...
		// secretText
		const secretText = this.add.bitmapText(
			185,
			6,
			"featured-item-font",
			"secret"
		...
		// medalGetText
		const medalGetText = this.add.bitmapText(
			69,
			29,
			"newgrounds-font",
			"MEDAL GET!"
	...
	/**
	 * Removes the medal event listener, calls the NGIO unlock and calls the popup to setup appearance.
	 * @param medal 
	 * @returns 
	 */
	unlockEventCallback(medal: medal) {
		if (!NGIO.isReady) {
			return;
		}
	...
	public setupPopup() {
		...
		console.log(
			`MEDAL GET: "${this.queue[0].name}" - ${this.queue[0].description}`
		);
	 ...
	 * 
	 * called as tween complete callback
	 */
	sequenceEndCallback() {
		this.queue.shift();
	...
	/**
	 * Adds each medal's event listener and loads it's icon.
	 */
	setupMedals() {
		medalData.forEach((medal: any) => {
			this.game.events.on("unlock-medal: " + medal.name, () => {
				this.unlockEventCallback(medal);
			});
